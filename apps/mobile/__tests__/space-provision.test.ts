 ...
 *   1. writeSpaceAccess (octospaces _access) — TOFU bootstrap write, must be FIRST
 *   2. seedSpaceObjectIndex (fiance content) — only runs after _access exists
 *   3. ownerEnsureSpaceKeyring (octospaces _keyring) — idem
 *   4. readSpaces / writeSpaces (_spaces list) — idem
 *   5. updateWeddingEntry — persist spaceId locally
 *
 * Ordering matters for the server's allowTofu design: the octospaces enricher
